...
public sealed class ChatWebSocketService
{
    ...
    string messageJson)
    {
        var plannerRequest = JsonSerializer.Deserialize<MessageRequest>(messageJson, JsonOptions)
        ?? throw new AppException(
            "WS_INVALID",
            "Invalid message",
            "Received invalid message format.");

        var request = plannerRequest.Data.Deserialize<PlannerRequest>(JsonOptions)
        ?? throw new AppException(
            "WS_INVALID_PLANNER",
    ...
    string messageJson)
    {
        ...
        try
        {
            ...
            try
            {
                messageRequest = JsonSerializer.Deserialize<MessageRequest>(
                    messageJson,
                    JsonOptions)!;
            }
            catch (Exception ex)
